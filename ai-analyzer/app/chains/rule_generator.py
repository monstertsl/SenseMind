...
- 前缀**必须**从以下白名单中选一个（这是 logstash 分类推送的关键词表，选错会导致告警不推送 AI 分析中心）：
  ...
  `sql injection`、`xss`、`rce`、`code injection`、`command injection`、`xxe`、`ssti`、`file upload`、`webshell`、`path traversal`、`directory traversal`、`lfi`、`rfi`、`file include`、`file read`、`file download`、`file write`、`ssrf`、`deserialization`、`brute force`、`weak password`、`default password`、`nmap`、`masscan`、`port scan`、`struts`、`log4j`、`spring`、`thinkphp`、`fastjson`、`shiro`、`weblogic`、`tomcat`、`exploit`、`trojan`、`beacon`、`backdoor`、`reverse shell`、`lateral`、`kerberos`、`data leak`、`dns tunnel`、`icmp tunnel`、`http tunnel`、`ddos`、`flood`、`privilege escalation`、`credential dump`、`mimikatz`、`powershell`、`cmd.exe`、`command execution`、`certutil`、`bitsadmin`、`mshta`、`rundll32`、`regsvr32`、`source leak`、`config leak`、`password leak`、`malware`、`ransomware`、`dropper`、`botnet`、`cobalt strike`、`metasploit`、`exfil`、`dga`、`jwt`、`auth bypass`、`password spray`、`unauthorized`、`nuclei`、`nikto`、`awvs`、`burp suite`、`syn flood`、`slowloris`、`cc attack`、`pass the hash`、`persistence`、`lolbin`、`virus`
