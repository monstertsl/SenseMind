...
- **low**: 规则匹配的 content 是明确的攻击特征（如 `etc/passwd`、`UNION SELECT`、`<?php`），正常流量几乎不会包含
- **medium**: 规则匹配的 content 有一定可能性出现在正常流量中（如普通关键词、常见路径）
- **high**: 规则过于宽泛（如只匹配 `GET`、`/`），极易误报
...
alert http $EXTERNAL_NET any -> $HOME_NET any (msg:"SenseMind AI: PHP Webshell Upload"; flow:established,to_server; content:"<?php"; nocase; sid:9000003; rev:1;)
```
